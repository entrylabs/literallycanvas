...
defineShape 'Image',
  ...
  toJSON: -> {@x, @y, imageSrc: @image.src, imageObject: @image, @scale, @flipX, @flipY, @width, @height, @rotate}
  fromJSON: (data) ->
    img = null
    ...
    createShape('Image', {
        ...
        flipX: data.flipX, flipY: data.flipY, width: data.width, height: data.height, rotate: data.rotate})
  move: ( moveInfo={} ) ->
    @x = @x - moveInfo.xDiff
    @y = @y - moveInfo.yDiff
